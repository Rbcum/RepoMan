...
namespace utils {
    ...
    QColor borderColor()
    {
        // QFusionStyle::drawPrimitive PE_Frame
        return qApp->palette().window().color().darker(140).lighter(108);
    }

    static bool paletteIsDark(const QPalette &pal)
    {
        return pal.color(QPalette::Window).lightnessF() <
               pal.color(QPalette::WindowText).lightnessF();
    }

    static bool isOverridingPalette(const Theme *theme)
    {
        if (theme->flag(Theme::DerivePaletteFromTheme)) return true;
    ...
    static void setMacAppearance(Theme *theme)
    {
        ...
        if (theme) Internal::forceMacAppearance(theme->flag(Theme::DarkUserInterface));
#else
        Q_UNUSED(theme)
#endif
    }
    ...
    {
        if (m_creatorTheme == theme) return;
    ...
    Theme::Theme(Theme *originTheme, QObject *parent)
        : QObject(parent), d(new ThemePrivate(*(originTheme->d)))
    {
    }

    Theme::~Theme()
    {
        if (this == m_creatorTheme) m_creatorTheme = nullptr;
    ...
    QStringList Theme::preferredStyles() const
    {
        ...
        if (d->preferredStyles.isEmpty() && flag(DarkUserInterface)) return {"Fusion"};
    ...
    QColor Theme::readNamedColorNoWarning(const QString &color) const
    {
        ...
        if (it != d->palette.constEnd()) return it.value();
        if (color == QLatin1String("style")) return {};
    ...
    {
        const auto it = d->palette.constFind(color);
        if (it != d->palette.constEnd()) return {it.value(), color};
        if (color == QLatin1String("style")) return {};
    ...
    void Theme::readSettingsInternal(QSettings &settings)
    {
        ...
        for (const QString &include : includes) {
            QDir dir = QFileInfo(d->fileName).dir();
            const QFile includedFile = QFile(dir.filePath(include));

            if (includedFile.exists()) {
                QSettings themeSettings(includedFile.fileName(), QSettings::IniFormat);
                readSettingsInternal(themeSettings);
            } else {
                qWarning("Theme \"%s\" misses include \"%s\".", qPrintable(d->fileName),
                    qPrintable(include));
            }
        }
    ...
    bool Theme::systemUsesDarkMode()
    {
        ...
        if (m_initialPalette) return paletteIsDark(*m_initialPalette);
    ...
    QPalette Theme::palette() const
    {
        ...
        if (!isOverridingPalette(this)) return pal;
