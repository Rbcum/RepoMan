...
NewTabPage::NewTabPage(QWidget *parent) : QWidget(parent), ui(new Ui::NewTabPage)
{
    ui->setupUi(this);
    auto filterModel = new ProjectListFilterModel(this);
    filterModel->setSourceModel(new ProjectListModel(this));
    ...
    connect(ui->listView, &QListView::doubleClicked, this, [this](const QModelIndex &index) {
        emit projectDoubleClicked(index.data(Qt::UserRole).value<RepoProject>());
    });
...
{
    return manifest.projectList.size();
}

QVariant ProjectListModel::data(const QModelIndex &index, int role) const
{
    const RepoProject &project = manifest.projectList[index.row()];
    ...
    QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    const RepoProject &project = index.data(Qt::UserRole).value<RepoProject>();
